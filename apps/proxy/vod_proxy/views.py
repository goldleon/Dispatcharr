    ...
    try:
        ...
        if content_type == 'movie':
            ...
            if content_obj is None and preferred_stream_id:
                ...
                if preferred_m3u_account_id:
                    rel = (
                        ...
                        .select_related('movie', 'm3u_account__user_agent', 'm3u_account__stream_profile', 'm3u_account__server_group')
                        .first()
                    )
                if rel is None:
                    rel = (
            ...
            candidates = list(
                ...
                .select_related('m3u_account__user_agent', 'm3u_account__stream_profile', 'm3u_account__server_group')
                .order_by('-m3u_account__priority', 'id')
            )
                ...
                if preferred_m3u_account_id:
                    rel = (
                        ...
                        .filter(stream_id=preferred_stream_id,
                                m3u_account_id=preferred_m3u_account_id,
                                m3u_account__is_active=True)
                        .select_related('episode', 'm3u_account__user_agent', 'm3u_account__stream_profile', 'm3u_account__server_group')
                        .first()
                    )
                if rel is None:
                    rel = (
    ...
    try:
        ...
        if not redis_client:
            ...
            default_profile = M3UAccountProfile.objects.filter(
                ...
                is_active=True,
                is_default=True
            ).select_related('m3u_account__user_agent', 'm3u_account__stream_profile', 'm3u_account__server_group').first()
                ...
                if existing_profile_id:
                    ...
                    try:
                        existing_profile = M3UAccountProfile.objects.select_related(
                            'm3u_account__user_agent', 'm3u_account__stream_profile', 'm3u_account__server_group'
                        ).get(
...
def _transform_url(original_url, m3u_profile):
    ...
    try:
        ...
        safe_replace_pattern = re.sub(r'\$<([^>]+)>', r'\\g<\1>', replace_pattern)
