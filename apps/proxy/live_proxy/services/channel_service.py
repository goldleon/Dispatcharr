...
class ChannelService:
    ...
    @staticmethod
    def initialize_channel(channel_id, stream_url, user_agent, transcode=False, stream_profile_value=None, stream_id=None, m3u_profile_id=None, channel_name=None, stream_name=None, channel_db_id=None):
        """
        ...
        Args:
            ...
            channel_db_id: Database ID of the channel (avoids DB lookup if already known)
        ...
        if success and proxy_server.redis_client:
            try:
                ...
                if channel_db_id:
                    update_data["channel_db_id"] = str(channel_db_id)
                else:
                    try:
                        from apps.channels.models import Channel
                        channel_obj = Channel.objects.filter(uuid=channel_id).first()
                        if channel_obj:
                            update_data["channel_db_id"] = str(channel_obj.id)
                    except Exception as e:
                        logger.debug(f"Could not resolve channel_db_id for {channel_id}: {e}")
